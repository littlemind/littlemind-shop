...
$(function() {
	...
	TransitionEffects	= (function() {
		...
		var $teWrapper		= $('#te-wrapper'),
			...
			if (!$.browser.webkit) {hasPerspective =false;};
		var	init			= function() {
				$teCover		= $teWrapper.find('div.te-cover');
				$teImages		= $teWrapper.find('div.te-images > img');
				imagesCount		= $teImages.length;
				...
				$teImgPosition	= $('#carousel-status').find('div'),
			...
			addPositionBubbles = function() {
				var count = 1,
					positions = $("div#carousel-status");
					positions.find("div:gt(0)").remove();
					var bubble = positions.find("div").removeClass("active"),
					paddingBubbles = (800-5-imagesCount*15)/2;
				while (count < imagesCount){
					bubble.clone().appendTo(positions);
					count++;
				}
				bubble.addClass("active");
				positions.css("padding-left", paddingBubbles);
			...
			updateImages	= function() {
				...
				var $last_img 	= $teImages.eq( last_img ),
					...
					$lastBubble = $teImgPosition.eq( last_img ),
					$currentBubble = $teImgPosition.eq( currentImg );
				
				$lastBubble.removeClass('active');
				$currentBubble.addClass('active');
				
				$("div.info").removeClass("description");
